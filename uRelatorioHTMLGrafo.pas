...
begin
  ...
  try
    // Coletar nodes únicos que passam no filtro
    ...
    html.Add('<html><head><meta charset="utf-8"><title>Grafo de Dependências</title>');
    ...
    html.Add('<h2 style="font-family: sans-serif">?? Grafo de Dependências entre Units</h2>');
    html.Add('<div id="grafo"></div><script>');

    if unidadesUnicas.Count = 0 then
    begin
      html.Add('<h2 style="font-family: sans-serif; color: #999;">? Nenhuma dependência entre units próprias foi encontrada.</h2>');
      html.Add('<p style="font-family: sans-serif;">O filtro ignorou bibliotecas padrão (System, Vcl, etc.) e não sobrou nenhum relacionamento entre suas próprias units. Tente revisar o código ou desativar o filtro para testar.</p>');
